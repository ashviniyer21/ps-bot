...
def get_all_moves():
    ...
    while (text.find("accuracy:") != -1):
        ...
        boosts = [0, 0, 0, 0, 0]
        if (text.find(",boosts:") < text.find(",type:\"") and text.find(",boosts:") < text.find(
                "secondary:") and text.find(",boosts:") != -1):
        ...
        num_hit = 1
        if (text.find("multihit:") < text.find(",type:\"") and text.find("multihit:") != -1 and text.find(
                "multihit:") < text.find("secondary:")):
            ...
            try:
                ...
                num_hit = 3
        status = "none"
        if (text.find("status:") < text.find(",type:\"") and text.find("status:") != -1 and text.find(
                "status:") < text.find("secondary:")):
...
def get_move(name):
    ...
    for i in range(len(all_moves)):
        if (all_moves[i].name.lower().replace(" ", "").replace("-", "") == name.lower() or all_moves[
            i].name.lower().replace(" ", "").replace("-", "") == name[:len(name) - 1].lower()):
            return all_moves[i]
    return all_moves[0]
